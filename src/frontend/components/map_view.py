...
def render_road_network_figure(
        ...
        for v, edge in graph.adj[u].items():
            if u < v:  # Undirected draw once
                ...
                if edge["is_blocked"]:
                    ...
                    ax.plot(x_vals, y_vals, color="#ef4444", linestyle="--", linewidth=2.5, alpha=0.9, zorder=2)
                    ...
                    ax.plot(x_vals, y_vals, color="#f59e0b", linestyle="-", linewidth=2.5, alpha=0.85, zorder=2)
                else:
                    # Clear normal road (Slate gray)
                    ax.plot(x_vals, y_vals, color="#334155", linestyle="-", linewidth=1.8, alpha=0.7, zorder=1)

    # 2. Highlight Computed Route Path (if provided)
    if route_path and len(route_path) > 1:
        rx = [graph.nodes[n]["x"] for n in route_path]
        ry = [graph.nodes[n]["y"] for n in route_path]
        # Glowing route underlay
        ax.plot(rx, ry, color="#06b6d4", linewidth=6.0, alpha=0.4, zorder=3)
        # Route main line
        ax.plot(rx, ry, color="#38bdf8", linewidth=3.2, linestyle="-", zorder=4)
    ...
    ax.set_title("Tactical Road Graph & Spatial Hazard Network", color="#f8fafc", fontsize=13, fontweight="bold", pad=12)
